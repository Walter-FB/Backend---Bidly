...
@router.post("/", status_code=201)
def crear_subasta(body: SubastaCreate, db: Session = Depends(get_db), current_user=Depends(get_current_client)):
    if current_user.get("rol") != "admin":
        raise HTTPException(403, detail={"message": "Solo admins pueden crear subastas", "code": "ADMIN_ONLY"})
    s = Subasta(
        fecha=body.fecha,
        hora=body.hora,
