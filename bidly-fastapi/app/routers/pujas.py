...
def colocar_puja(body: PujaCreate, db: Session = Depends(get_db)):
    ...
    # Verificar que tenga al menos un medio de pago verificado
    medio_verificado = (
        db.query(MedioPago)
        .filter(MedioPago.cliente == asistente.cliente, MedioPago.verificado == "si")
        .first()
    )
    if not medio_verificado:
        raise HTTPException(
            403,
            detail={"message": "No tenés medios de pago verificados. Registrá uno y esperá la aprobación.", "code": "NO_PAYMENT"},
        )

    # Validar tope de cheque (solo si el único medio verificado es cheque)
    if medio_verificado.tipo == "cheque" and medio_verificado.montocheque:
        otro_verificado = (
            db.query(MedioPago)
            .filter(
                MedioPago.cliente == asistente.cliente,
                MedioPago.verificado == "si",
                MedioPago.tipo != "cheque",
            )
            .first()
        )
        if not otro_verificado:
            pendientes = (
                db.query(func.sum(RegistroPago.importe_total))
                .join(RegistroDeSubasta, RegistroPago.registro == RegistroDeSubasta.identificador)
                .filter(
                    RegistroDeSubasta.cliente == asistente.cliente,
                    RegistroPago.estado == "pendiente",
                )
                .scalar()
            ) or Decimal("0")
            tope = Decimal(str(medio_verificado.montocheque))
            if pendientes + importe > tope:
                raise HTTPException(
                    422,
                    detail={
                        "message": f"Superás el tope de garantía del cheque. Pendientes: ${float(pendientes):.2f}, tope: ${float(tope):.2f}",
                        "code": "CHEQUE_LIMIT",
                        "tope": float(tope),
