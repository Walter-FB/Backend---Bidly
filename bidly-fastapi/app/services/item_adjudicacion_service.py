...
def _adjudicar(item: ItemCatalogo, puja: Puja, db: Session) -> ItemCatalogo:
    ...
    # Notificación al vendedor con desglose de la venta
    if prod and prod.duenio:
        neto = float(puja.importe or 0) - float(item.comision or 0)
        ...
        cobro_info = (
            f" Se acreditará en tu cuenta {cuenta_cobro.numerocuenta} ({cuenta_cobro.banco})."
            if cuenta_cobro and cuenta_cobro.numerocuenta
            else ""
        )
        notificacion_service.crear(
            prod.duenio,
            "venta",
            f"¡Tu producto se vendió por ${float(puja.importe or 0):.2f}! "
            f"Comisión Bidly: ${float(item.comision or 0):.2f}. "
            f"Neto a recibir: ${neto:.2f}.{cobro_info}",
            db,
        )
